...
void ASlideGameMode::TestCast() {
 auto Space=[](bool Repeat=false){FSlateApplication::Get().ProcessKeyDownEvent(FKeyEvent(EKeys::SpaceBar,FModifierKeysState(),0,Repeat,0,0));FSlateApplication::Get().ProcessKeyUpEvent(FKeyEvent(EKeys::SpaceBar,FModifierKeysState(),0,false,0,0));};
 ...
 if(SmokeStep==0&&Elapsed>1){GoTo(0);SmokeStep=1;}
  ...
  const int32 Page=PageIndex;
  Space();
  if(!Check(!CastPlayer->Paused&&CastPlayer->Time==Before&&PageIndex==Page&&bCommandDesktop,TEXT("Space resumes the recording without advancing the slide")))return;
  CastPlayer->Tick(.25f);Space();const float PausedAt=CastPlayer->Time;CastPlayer->Tick(1);Space(true);
  if(!Check(CastPlayer->Paused&&CastPlayer->Time==PausedAt&&PausedAt>Before&&PageIndex==Page,TEXT("Space pauses playback and ignores held-key repeats")))return;
  ...
  CastPlayer->Seek(CastPlayer->Duration);Space();
  if(!Check(CastPlayer->Time==0&&!CastPlayer->Paused&&bCommandDesktop,TEXT("Space restarts a finished recording")))return;
