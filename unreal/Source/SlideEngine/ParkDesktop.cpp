...
void ASlideGameMode::EndPlay(const EEndPlayReason::Type Reason) {
 ...
 TimeMenu.Reset();ElapsedTimeField.Reset();ApplyTimeButton.Reset();ParkMenu.Reset();LogoutButton.Reset();LoginButtonWidget.Reset();WorkstationField.Reset();LoginHUD.Reset();DesktopHUD.Reset(); Super::EndPlay(Reason);
}
