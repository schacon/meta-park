...
void ASlideGameMode::ToggleTerminal() {
...
void ASlideGameMode::TestTerminal() {
 ...
 if(SmokeStep==0&&Elapsed>1){GoTo(0);SmokeStep=1;}
 else if(SmokeStep==1&&MapPhase==EMapPhase::Slide){HandleParkKey(EKeys::SpaceBar);SmokeStep=2;}
 else if(SmokeStep==2&&Terminal->IsRaised()&&Terminal->Command().Len()>=4) {
  if(!Check(Index==0&&MapPhase==EMapPhase::Slide&&Terminal->Command().Len()<12&&!Terminal->OutputReady()&&Terminal->ScreenFillsViewport(),TEXT("Space raises a close-framed display and types incrementally")))return;
 ...
 } else if(SmokeStep==4&&Travel>.25f){HandleParkKey(EKeys::SpaceBar);SmokeStep=5;}
 else if(SmokeStep==5&&Terminal->IsHidden()) {
  if(!Check(Index==0&&MapPhase==EMapPhase::Slide,TEXT("second Space retracts without changing slides")))return;
 ...
 } else if(SmokeStep==6&&Travel>.25f){HandleParkKey(EKeys::SpaceBar);SmokeStep=7;}
 else if(SmokeStep==7&&Terminal->IsRaised()) {
  ...
  HandleParkKey(EKeys::SpaceBar);SmokeStep=9;
 } else if(SmokeStep==9&&MapPhase==EMapPhase::Overview) {
  if(!Check(Terminal->IsHidden(),TEXT("Space still navigates in other areas")))return;
