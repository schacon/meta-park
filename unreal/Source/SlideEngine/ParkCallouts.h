...
public:
 SLATE_BEGIN_ARGS(SParkCallouts) : _Fade(1.f) {}
  SLATE_ATTRIBUTE(float,Fade)
  SLATE_ARGUMENT(APlayerController*,Controller)
  SLATE_ARGUMENT(FGetParkPins,Pins)
  SLATE_ARGUMENT(FChooseParkPin,Choose)
 SLATE_END_ARGS()
 void Construct(const FArguments& Args) { Fade=Args._Fade; Controller=Args._Controller; Pins=Args._Pins; Choose=Args._Choose; }
 ...
 virtual int32 OnPaint(const FPaintArgs&,const FGeometry& G,const FSlateRect&,FSlateWindowElementList& Out,int32 Layer,const FWidgetStyle&,bool) const override {
  ...
  const FLinearColor Ink(.018,.028,.025,Fade.Get()),Paper(.66,.67,.63),Green(.15,.61,.25);
  const auto* Brush=FCoreStyle::Get().GetBrush("WhiteBrush");
  auto Box=[&](FVector2D P,FVector2D S,FLinearColor Color,int32 Z){Color.A=Fade.Get();FSlateDrawElement::MakeBox(Out,Layer+Z,G.ToPaintGeometry(S,FSlateLayoutTransform(P)),Brush,ESlateDrawEffect::None,Color);};
  auto Text=[&](FString Value,FVector2D P,int32 Size,int32 Z,FLinearColor Color=FLinearColor(.018,.028,.025)){Color.A=Fade.Get();FSlateDrawElement::MakeText(Out,Layer+Z,G.ToPaintGeometry(FVector2D(220,24),FSlateLayoutTransform(P)),Value,FCoreStyle::GetDefaultFontStyle("Mono",Size),ESlateDrawEffect::None,Color);};
  for(const auto& Pin:Pins()) {
   ...
   Text(Pin.SlideIndex>=0?FString::Printf(TEXT("[%d]"),Pin.SlideIndex+1):TEXT("—"),P+FVector2D(164,8),14,5);
   Text(Pin.Name,P+FVector2D(12,29),14,5);
   Box(P+FVector2D(12,54),FVector2D(Pin.Status.Len()*10+12,19),Pin.StatusColor,5);
   Text(Pin.Status,P+FVector2D(18,53),13,6,Pin.StatusInk);
