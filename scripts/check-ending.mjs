...
const dir=await mkdtemp(join(tmpdir(),'park-ending-test-'));
try {
 for(let i=1;i<=8;i++) {
 ...
 if(process.argv.includes('--packaged')) {
  ...
  const staged=join(container,'ending-test.json');await copyFile(manifest,staged);
  try {result=spawnSync(join(bundle,'Contents/MacOS/SlideEngine'),['-windowed','-SlideSmokeTest','-EndingTest',`-SlideManifest=${staged}`,'-stdout',...flags],{stdio:'inherit',timeout:180000});}
  finally {await rm(staged,{force:true});}
 } else result=spawnSync(process.execPath,['scripts/unreal.mjs','smoke','-EndingTest',`-SlideManifest=${manifest}`,...flags],{stdio:'inherit'});
