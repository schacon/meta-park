...
const dir=await mkdtemp(join(tmpdir(),'park-serializer-test-'));
try {
 for(let i=1;i<=8;i++) {
  const folder=join(dir,String(i).padStart(2,'0'));await mkdir(folder);
  await writeFile(join(folder,'01-intro.mdx'),`# Station ${i}`);
 }
 await copyFile('examples/git-meta/serializer.mdx',join(dir,'05/02-serializer.mdx'));
 await writeFile(join(dir,'05/03-after.mdx'),'# After serialization');
 ...
 if(process.argv.includes('--packaged')) {
  ...
  const staged=join(container,'serializer-test.json');await copyFile(manifest,staged);
  try {result=spawnSync(join(bundle,'Contents/MacOS/SlideEngine'),['-windowed','-SlideSmokeTest','-SerializerTest',`-SlideManifest=${staged}`,'-stdout',...flags],{stdio:'inherit',timeout:180000});}
  finally {await rm(staged,{force:true});}
 } else result=spawnSync(process.execPath,['scripts/unreal.mjs','smoke','-SerializerTest',`-SlideManifest=${manifest}`,...flags],{stdio:'inherit'});
