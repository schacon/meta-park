...
test('example compiles seven slides and reusable animated MDX components',async()=>{
...
test('island habitat shuffle is deterministic, complete, and preserves the narrative',async()=>{
 ...
 const out=await compileDeck('examples/git-meta/deck.mdx','examples/git-meta/layout.json');
 assert.deepEqual(out.slides.map(s=>s.id),['opening','context','targets','authoring','storage','merging','scale']);
 assert.deepEqual(out.slides.map(s=>s.habitat),a.route.slice(0,7).map(h=>h.id));
 for(let i=0;i<7;i++) assert.deepEqual(out.slides[i].position,[...a.route[i].position.slice(0,2),a.route[i].position[2]+3200]);
...
test('every slide has one numbered destination, including all three facilities',async()=>{
 ...
 const layout=JSON.parse(await readFile('examples/git-meta/layout.json','utf8'));
 const out=await compileDeck('examples/git-meta/deck.mdx','examples/git-meta/layout.json');
 assert.deepEqual(out.slides.map(s=>s.card.code),['ENC-01','ENC-02','ENC-03','ENC-04','VC','HELI','GATE']);
 assert.equal(new Set(out.slides.map(s=>s.card.slide)).size,7);
 assert.equal(new Set(out.slides.map(s=>JSON.stringify(s.card.view.eye))).size,7);
