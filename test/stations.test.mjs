...
test('invalid native components and empty stations fail with an actionable source path',async t=>{
 ...
 await assert.rejects(compileStations(dir,layout),/02-bad.mdx: Computer needs/);
 await writeFile(file,'# Mixed\n\n<Computer><prompt>hello</prompt><output>OK</output></Computer>');
 await assert.rejects(compileStations(dir,layout),/02-bad.mdx: Unsupported MDX element: computer/);
 await writeFile(file,'<Unknown />');
 await assert.rejects(compileStations(dir,layout),/02-bad.mdx:.*Unknown/);
 await rm(file);await rm(join(dir,'01/01-intro.mdx'));
 await assert.rejects(compileStations(dir,layout),/01: station needs/);
});
